...
func (s *SUser) UpdateUserByID(id uint64, ctx context.Context) error {
	user := new(model.UserSubject)
	_, err := g.MysqlDB.ExecContext(ctx, "update user_subject set username=?,password=?,updatetime=?where id=?",
		user.Username,
		user.Password,
		time.Now(),
		id)
