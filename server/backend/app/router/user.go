...
func (r *UserRouter) InitUserSignRouter(router *gin.RouterGroup) gin.IRouter { //创建登录注册的路由
	...
	userApi := api.User()
	{
		...
		userRouter.POST("/update", userApi.Sign().UpdateUser)
	}
...
func (r *UserRouter) InitUserInfoRouter(router *gin.RouterGroup) gin.IRoutes { //创建实现用户其他功能的路由
	...
	userApi := api.User()
	{
		userRouter.POST("/create_collection", userApi.Collect().CreateCollect)     //调用Register中间件
		userRouter.POST("/delete_collection", userApi.Collect().DeleteCollectByID) //调用登陆中间件
	}
	return userRouter
}
